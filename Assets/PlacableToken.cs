...
public class PlacableToken : MonoBehaviour, IInteractable
{
    ...
    [SerializeField] private float maximumTreshold;
    private ColumnPreview closest = null;
    ...
    private void FixedUpdate()
    {
        if (grabbers.Count > 0)
        {
            foreach (ColumnPreview preview in ColumnPreview.ColumnPreviews)
            {
                preview.SetPreview(false);
                if (closest == null || Vector3.Distance(transform.position, preview.transform.position) <
            ...
            if (Vector3.Distance(closest.transform.position, transform.position) < maximumTreshold)
            {
                closest.SetPreview(true);
            }
            else
            {
                closest = null;
